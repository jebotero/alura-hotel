...
public class Busqueda extends JFrame {
	...
	public Busqueda() {
		...
		Huespedes = new JTable();
		Huespedes.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		Huespedes.setFont(new Font("Roboto", Font.PLAIN, 16));
		panel.addTab("Huéspedes", new ImageIcon(Busqueda.class.getResource("/imagenes/pessoas.png")), Huespedes, null);
		modeloHuespedes = (DefaultTableModel) Huespedes.getModel();
		modeloHuespedes.addColumn("Numero de Huesped");
		...
		modeloHuespedes.addColumn("Numero de Reserva");
		LlenarTablaHuespedes();
		
		Reservas = new JTable();
		Reservas.setFont(new Font("Roboto", Font.PLAIN, 16));
		panel.addTab("Reservas", new ImageIcon(Busqueda.class.getResource("/imagenes/reservado.png")), Reservas, null);
		...
		Reservas.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
		...
		btnEditar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				int filaReservas = Reservas.getSelectedRow();
				int filaHuespedes = Huespedes.getSelectedRow();
		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas >= 0) {
					...
					reserva = Reservas.getValueAt(filaReservas, 0).toString();
					int confirmar = JOptionPane.showConfirmDialog(null, "¿Desea Eliminar los datos?"); 

					if(confirmar == JOptionPane.YES_OPTION){

						String valor = Reservas.getValueAt(filaReservas, 0).toString();			
					...
					huespedes = Huespedes.getValueAt(filaHuespedes, 0).toString();
					int confirmarh = JOptionPane.showConfirmDialog(null, "¿Desea Eliminar los datos?"); 

					if(confirmarh == JOptionPane.YES_OPTION){

						String valor = Huespedes.getValueAt(filaHuespedes, 0).toString();			
...
private void limpiarTabla() {
	((DefaultTableModel) Huespedes.getModel()).setRowCount(0);
	((DefaultTableModel) Reservas.getModel()).setRowCount(0);
}
private void LlenarTablaReservas() {
	...
	try {
		for (Reserva reservas : reserva) {
			modelo.addRow(new Object[] { reservas.getId(), reservas.getFechaEntrada(), reservas.getFechaSalida(), reservas.getValor(), reservas.getForma_pago() });
		}
	} catch (Exception e) {
		throw e;
	}
...
private void ActualizarReservas() {		
	Optional.ofNullable(modelo.getValueAt(Reservas.getSelectedRow(), Reservas.getSelectedColumn()))
    .ifPresentOrElse(fila -> {
    	
    	Date fechaE = Date.valueOf(modelo.getValueAt(Reservas.getSelectedRow(), 1).toString());		
    	Date fechaS = Date.valueOf(modelo.getValueAt(Reservas.getSelectedRow(), 2).toString());
		String valor = (String) modelo.getValueAt(Reservas.getSelectedRow(), 3);
		String formaPago = (String) modelo.getValueAt(Reservas.getSelectedRow(), 4);
		Integer id = Integer.valueOf(modelo.getValueAt(Reservas.getSelectedRow(), 0).toString());
...
private void ActualizarHuesped() {		
	Optional.ofNullable(modeloHuespedes.getValueAt(Huespedes.getSelectedRow(), Huespedes.getSelectedColumn()))
    .ifPresentOrElse(filaHuesped -> {
    	
    	String nombre = (String) modeloHuespedes.getValueAt(Huespedes.getSelectedRow(), 1);
    	String apellido = (String) modeloHuespedes.getValueAt(Huespedes.getSelectedRow(), 2);
    	Date fechaN = Date.valueOf(modeloHuespedes.getValueAt(Huespedes.getSelectedRow(), 3).toString());
		String nacionalidad = (String) modeloHuespedes.getValueAt(Huespedes.getSelectedRow(), 4);
		String telefono = (String) modeloHuespedes.getValueAt(Huespedes.getSelectedRow(), 5);
		Integer idReserva = Integer.valueOf(modeloHuespedes.getValueAt(Huespedes.getSelectedRow(), 6).toString());
		Integer id = Integer.valueOf(modeloHuespedes.getValueAt(Huespedes.getSelectedRow(), 0).toString());
